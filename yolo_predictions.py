...
class YOLO_Pred():
    ...
    def predictions(self,realimage):
        row, col, d = realimage.shape
        max_rc = max(row, col)
        image = np.zeros((max_rc, max_rc, 3), dtype=np.uint8)
        image[0:row, 0:col, :] = realimage
        INPUT_WH_YOLO = 224
        ...
        # Width and Height of the image (image)
